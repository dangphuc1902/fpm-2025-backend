...
public class ParsedNotificationConsumer {
    ...
    public void handleNotificationParsed(ParsedNotificationEvent event) {
        log.info("[Local/Spring Received] ParsedNotificationEvent: userId={} amount={} type={}",
        ...
        try {
            ...
            log.info("[Local/Spring Processed] Auto-transaction created for userId={} walletId={}", event.getUserId(), walletId);

        } catch (Exception e) {
            log.error("[Local/Spring Failed] Failed to process parsed notification for userId={}, error={}",
