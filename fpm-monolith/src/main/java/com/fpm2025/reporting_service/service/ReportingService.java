...
import com.fpm2025.reporting_service.domain.*;
import com.fpm2025.reporting_service.domain.model.Budget;
import com.fpm2025.reporting_service.domain.model.CategorySummary;
...
import com.fpm2025.reporting_service.repository.BudgetRepository;
import com.fpm2025.reporting_service.repository.CategorySummaryRepository;
import com.fpm2025.reporting_service.repository.ReportRepository;
import com.fpm2025.reporting_service.repository.TransactionSummaryRepository;
import com.fpm2025.reporting_service.dto.response.BudgetComparisonItem;
import com.fpm2025.reporting_service.dto.response.ChartDataResponse;
...
import com.fpm2025.reporting_service.exception.ResourceNotFoundException;
...
public class ReportingService {
    ...
    private final com.fpm2025.reporting_service.repository.ExportJobRepository exportJobRepository;
    ...
    public Long submitExportJob(Long userId, com.fpm2025.reporting_service.dto.request.ReportRequest request) {
        log.info("Submitting async export job for user: {}, format: {}", 
            userId, request.getFormat());
        
        com.fpm2025.reporting_service.domain.model.ExportJob job = com.fpm2025.reporting_service.domain.model.ExportJob.builder()
            .userId(userId)
            .format(request.getFormat())
            .period(request.getStartDate().getYear() + "-" + String.format("%02d", request.getStartDate().getMonthValue()))
            .status(com.fpm2025.reporting_service.domain.valueobject.ExportStatus.PENDING)
            .build();
            
        com.fpm2025.reporting_service.domain.model.ExportJob savedJob = exportJobRepository.save(job);
    ...
    @org.springframework.transaction.annotation.Transactional
    public void processExportJobAsync(Long jobId, com.fpm2025.reporting_service.dto.request.ReportRequest request) {
        log.info("Processing export job {} asynchronously", jobId);
        
        com.fpm2025.reporting_service.domain.model.ExportJob job = exportJobRepository.findById(jobId)
        ...
        try {
            ...
            Long userId = request.getUserId();
            com.fpm2025.reporting_service.domain.valueobject.ExportFormat format = request.getFormat();
            ...
            List<com.fpm2025.reporting_service.domain.TransactionData> transactions = transactionClient
                .getTransactionsByDateRange(userId, startDate, endDate);
            List<com.fpm2025.reporting_service.domain.WalletData> wallets = walletClient.getUserWallets(userId);
            com.fpm2025.reporting_service.domain.MonthlyStatistics stats = calculateMonthlyStatistics(transactions, wallets, startDate, endDate);

            byte[] reportData = reportGenerator.generate(transactions, stats, wallets, 
                com.fpm2025.reporting_service.domain.ReportFormat.valueOf(format.name()));
    ...
    public com.fpm2025.reporting_service.domain.model.ExportJob getExportJobStatus(Long jobId, Long userId) {
        com.fpm2025.reporting_service.domain.model.ExportJob job = exportJobRepository.findById(jobId)
            .orElseThrow(() -> new com.fpm2025.reporting_service.exception.ResourceNotFoundException("Job not found: " + jobId));
