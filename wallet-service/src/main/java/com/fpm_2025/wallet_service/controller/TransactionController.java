...
public class TransactionController {
	...
	@Valid @RequestBody CreateTransactionRequest request, @AuthenticationPrincipal Long userId) {
	...
	public ResponseEntity<BaseResponse<PageResponse<TransactionResponse>>> getUserTransactions(
			...
			@RequestParam(defaultValue = "20") int size, @RequestParam(defaultValue = "transactionDate") String sortBy,
		...
		PageResponse<TransactionResponse> pageResponse = PageResponse.<TransactionResponse>builder()
				.content(transactions.getContent()).pageNumber(transactions.getNumber())
				.pageSize(transactions.getSize()).totalElements(transactions.getTotalElements())
				.totalPages(transactions.getTotalPages()).last(transactions.isLast()).build();
	...
	public ResponseEntity<BaseResponse<PageResponse<TransactionResponse>>> getTransactionsByType(
			@PathVariable CategoryType type, @AuthenticationPrincipal Long userId,
	...
	public ResponseEntity<BaseResponse<TransactionResponse>> updateTransaction(@PathVariable Long id,
			@Valid @RequestBody UpdateTransactionRequest request, @AuthenticationPrincipal Long userId) {
	...
	public ResponseEntity<BaseResponse<PageResponse<TransactionResponse>>> getWalletTransactions(
			@PathVariable Long walletId, @AuthenticationPrincipal Long userId,
			@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int size) {
	...
	public ResponseEntity<BaseResponse<BigDecimal>> getTotalAmount(@RequestParam CategoryType type,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime startDate,
			@RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE_TIME) LocalDateTime endDate,
			@AuthenticationPrincipal Long userId) {
	...
	public ResponseEntity<BaseResponse<Long>> getTransactionCount(@AuthenticationPrincipal Long userId) {
